...
function Editar(btn) {
  ...
  convocatoria.form.append(
    '<input id="convocatoria_method" type="hidden" name="_method" value="PUT" />'
  );
  ...
  $.get(_route, function (p) {
    ...
    document.querySelector(`${form} input[name="referencia"]`).value =
      p.referencia;
    document.querySelector(`${form} input[name="id"]`).value = p.idnoti;
    document.querySelector(`${form} input[name="fecha"]`).value =
      p.fecha_formatted;
    document.querySelector(`${form} select[name="published"]`).value =
      p.published;
    ...
    if (p.resultados) {
      $('#form-convocatorias input[name="resultados"]')
        .next()
        .attr("href", p.resultados);
    ...
    $('#form-convocatorias input[name="comunicados"]').next().hide();
    if (p.comunicados) {
      $('#form-convocatorias input[name="comunicados"]')
        .next()
        .attr("href", p.comunicados);
      $('#form-convocatorias input[name="comunicados"]').next().show();
    }
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      ...
      $.each(jqXHR.responseJSON.errors, function (key, value) {
        $.each(value, function (errores, eror) {
          $(`#convocatoria-${key}-error`).append(
            "<li class='error-block'>" + eror + "</li>"
          );
        });
      });
    },
