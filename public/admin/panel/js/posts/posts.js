...
function datatablePosts(categoryId)
{
    $postsTable = $('#posts-datatable').DataTable({
        ...
        serverSide: true,
        destroy:true,
        ajax: `/admin/publicaciones-datatable?category_id=${categoryId}&role_id=${roleId}`,
        columns: [
            {data:'id', name: 'info_informacion.in_id_informacion', searchable: false},
            {data:'title', name: 'info_informacion.vc_titulo_informacion', searchable: true},
            {data:'summary', name: 'info_informacion.vc_resumen_informacion', searchable: false},
            {data:'date', name: 'info_informacion.created_at', searchable: false},
            {data:'photo', name: 'info_informacion.foto', searchable: false},
...
function Editar(btn){
    ...
    cleanError();
    cleanModal();
    post.modalTitle.text("Editar Noticia");
    ...
    $.get(_route, function(p){
       ...
       //$('#post_id').val(p.id);
        ...
        document.querySelector(`${form} input[name="fecha"]`).value = p.fecha; 
        addSummernoteEditorMini($(`#content-post`));
      ...
      $.ajax({
         ...
         data: data,
            success: function(re){
                ...
                unlockWindow();
                $postsTable.ajax.reload();
                Swal.fire(`Operación Exitosa`, `Se ha eliminado la noticia`, `success`);
            },
            error:function(jqXHR, textStatus, errorThrown)
            {
               //$('body').modalmanager('removeLoading');
...
$('#btnCrearpublicacion').on('click', function(){
  ...
  post.modalTitle.text("Crear Noticia");
	post.btnUpdate.hide();
  post.btnSave.show();
...
post.btnUpdate.on('click',function(event){
    ...
    $.ajax({
        ...
        success: function(e){
            ...
            unlockWindow();
            Swal.fire(`Operación Exitosa`, `Se ha actualizado correctamente la noticia`, `success`);
            $postsTable.ajax.reload();
            $('#modalCrearNoticia').modal('hide');
        },
        error:function(jqXHR, textStatus, errorThrown)
        {
            ...
            Swal.fire(`Ha ocurrido un error`, `No se ha actualizado la noticia. Puede ser debido a un error procesando la/las imágenes. Inténtelo de nuevo.`, `warning`);
...
post.btnSave.on('click',function(event){
	...
	$.ajax({
		...
		success: function(e){
			//$('body').modalmanager('removeLoading');
            Swal.fire(`Operación Exitosa`, `Se ha creado correctamente la noticia`, `success`);
            $postsTable.ajax.reload();
            $(`#modalCrearNoticia`).modal('hide');
            unlockWindow();
		...
		error: function(jqXHR, textStatus, errorThrown)
		{
            ...
            Swal.fire(`Ha ocurrido un error`, `No se ha creado la noticia. Puede ser debido a un error procesando la/las imágenes. Inténtelo de nuevo.`, `warning`);
        }
	});
});
