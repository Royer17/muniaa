...
(function(factory) {
    ...
    let $calendarTitle = $('.calendar').data('title');
    ...
    EvoCalendar = (function() {
        function EvoCalendar(element, settings) {
            ...
            var nameMonth = _.initials.dates[_.options.language].months;
            // these are human-readable month name labels, in order
            _.$cal_months_labels = nameMonth;
    ...
    EvoCalendar.prototype.buildCalendar = function(val, new_month, new_year) {
        ...
        var nameDays = _.initials.dates[_.options.language].daysShort;
        var firstDayOfWeekName = _.initials.dates[_.options.language].daysShort.map(function(obj) {return obj}).indexOf(_.defaults.firstDayOfWeek);
        ...
        var firstDay = new Date(new_year, new_month).getDay();
        ...
        function buildMainHTML() {
            var mainHTML = '<div class="calendar-sidebar"></div><div class="calendar-inner"></div>';
        ...
        function buildSidebarHTML() {
            sidebarHTML = '<div class="calendar-year"><button class="icon-button" year-val="prev"><span class="chevron-arrow-left"></span></button>&nbsp;<p>'+new_year+ '&nbsp; '+$calendarTitle+'</p><button class="icon-button" year-val="next"><span class="chevron-arrow-right"></span></button></div>';
            ...
            for (var i = 0; i < 9; i++) {
                ...
                for (var j = 0; j <= 6; j++) { 
                    ...
                    if (day <= monthLength && (i > 0 || j >= startingDay)) {
                        var monthNumber = (+new_month + 1) > 9 ? (+new_month + 1) : '0' + (+new_month +1);
                        var thisDay = _.formatDate(new Date(monthNumber +'/'+ day +'/'+ new_year), _.options.format, 'en');
